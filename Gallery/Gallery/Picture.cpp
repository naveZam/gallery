...
void Picture::setCreationDateNow()
{
	...
	struct tm timeinfo;
	localtime_s(&timeinfo, &now); // Get the local time as a tm structure
	char buffer[80];
	strftime(buffer, sizeof(buffer), "%d/%m/%Y %H:%M:%S", &timeinfo); // Format the time as a string
	m_creationDate = buffer;
}
